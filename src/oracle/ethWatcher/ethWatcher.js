...
const HOME_MAX_FETCH_RANGE_SIZE = parseInt(
  process.env.HOME_MAX_FETCH_RANGE_SIZE,
  10
);
...
const validatorAddress = ethers.utils.computeAddress(
  `0x${VALIDATOR_PRIVATE_KEY}`
);
...
async function resetFutureMessages(queue) {
  ...
  if (messageCount) {
    logger.info(
      `Filtering ${messageCount} reloaded messages from queue ${queue.name}`
    );
...
async function sendSignFundsTransfer(event) {
  ...
  const [x, y, threshold, parties] = await Promise.all([
...
async function processEpochStart(event) {
  ...
  logger.info("Validators", await bridge.getValidators(epoch));
  isCurrentValidator = (await bridge.getValidators(epoch)).includes(
    validatorAddress
  );
...
async function initialize() {
  ...
  const saved =
    parseInt(await redis.get("homeBlock"), 10) + 1 ||
    parseInt(HOME_START_BLOCK, 10);
  if (epochStart > saved) {
    logger.info(
      `Data in db is outdated, starting from epoch ${epoch}, block #${epochStart}`
    );
    ...
    blockNumber = saved;
    foreignNonce[epoch] =
      parseInt(await redis.get(`foreignNonce${epoch}`), 10) || 0;
  }
...
async function loop() {
  ...
  const endBlock = Math.min(
    latestBlockNumber,
    blockNumber + HOME_MAX_FETCH_RANGE_SIZE - 1
  );
  ...
  for (
    let curBlockNumber = blockNumber, i = 0;
    curBlockNumber <= endBlock;
    curBlockNumber += 1
  ) {
    ...
    while (
      i < bridgeEvents.length &&
      bridgeEvents[i].blockNumber === curBlockNumber
    ) {
      ...
      switch (event.name) {
        case "NewEpoch":
          if (
            (await bridge.getValidators(event.values.newEpoch)).includes(
              validatorAddress
            )
          ) {
        ...
        case "NewEpochCancelled":
          if (
            (await bridge.getValidators(event.values.epoch)).includes(
              validatorAddress
            )
          ) {
        ...
        case "EpochStart":
          ...
          epochTimeIntervalsQueue.send({
            ...
            startTime:
              (await retry(() => getBlockTimestamp(curBlockNumber))) * 1000,
            epoch,
          });
    ...
    if (rangeOffset === 0) {
      ...
      if (
        isCurrentValidator &&
        lastTransactionBlockNumber > curBlockNumber - rangeSize
      ) {
        logger.info(
          "Sending message to start signature generation for the ended range"
        );
        await sendStartSign();
      }
    }
